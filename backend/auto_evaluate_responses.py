...
"""
    ...
    # Add voice response context if available
    voice_response = experience.get('voice_response', '')
    has_voice = bool(voice_response and voice_response.strip())

    if has_voice:
        evaluation_prompt += f'\n**Voice Response Generated by AI**:\n"{voice_response}"\n'

    evaluation_prompt += "\n**Available Widget Types**: KPI, Trend, Table, Gauge, Alert, Comparison, Chart, Distribution\n"

    # Build JSON schema — include voice_evaluation block only if voice exists
    voice_json_block = ""
    if has_voice:
        voice_json_block = """,
  "voice_evaluation": {
    "quality_rating": "GOOD" or "POOR",
    "confidence": 0.0-1.0,
    "reasoning": "Why this voice response is good or poor",
    "dimension_scores": {
      "groundedness": 0.0-1.0,
      "conciseness": 0.0-1.0,
      "directness": 0.0-1.0,
      "specificity": 0.0-1.0,
      "tts_friendliness": 0.0-1.0
    }
  }"""
...
{{
  ...
  "suggested_improvements": ["specific actionable suggestion 1", "suggestion 2"]{voice_json_block}
}}
```
...
- Consider context: Intent type and domains inform what's appropriate"""
    ...
    if has_voice:
        evaluation_prompt += """
- **Voice Quality**: Evaluate the voice response text on 5 dimensions:
  - groundedness: Does it cite actual data/numbers from the query context?
  - conciseness: Is it 1-3 sentences, no filler words?
  - directness: Does it lead with the key insight, not preamble?
  - specificity: Does it name equipment, values, units?
  - tts_friendliness: No markdown, abbreviations, or symbols that TTS would mangle?"""
...
def create_rating(experience: Dict, evaluation: Dict):
    ...
    try:
        ...
        # Extract voice evaluation if Claude provided it
        ...
        api_payload = {
            ...
            "correction": correction[:500],  # API limit is 500 chars
        ...
        # Add voice evaluation fields if Claude evaluated the voice response
